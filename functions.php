...
function twentytwenty_get_elements_array() {
	...
	$elements = array(
		...
		'header-footer' => array(
			'accent'     => array(
				'color'            => array( 'body:not(.overlay-header) .primary-menu > li > a', 'body:not(.overlay-header) .primary-menu > li > .icon', '.modal-menu a', '.footer-menu a, .footer-widgets a', '#site-footer .wp-block-button.is-style-outline', '.wp-block-pullquote:before', '.singular:not(.overlay-header) .entry-header a', '.archive-header a'),
...
function news_shortcode( $atts ) {
    extract( shortcode_atts( array( 'limit' => -1, 'type' => 'news'), $atts ) );
	
	$paged = get_query_var('paged') ? get_query_var('paged') : 1;  
	
    $q = new WP_Query(  array ( 'posts_per_page' => $limit, 'post_status' => 'publish', 'post_type' => $type,  'paged' => $paged,  ) );
	$list = ' ';
	
    while ( $q->have_posts() ) { 
		$q->the_post();
        $list .= '<div class="item"><div class="details">'
		. '<div class="news-images"><a href="' . get_permalink() . '">' . get_the_post_thumbnail($page->ID, 'listing-thumb') . '</a></div>'
		. '<div class="content">'
        . '<h3>' . get_the_title() . '</h3>'
		. '<div class="date">' . get_the_time( get_option( 'date_format' ) ) . '</div>' 
        . '<div class="other-info"><a href="' . get_permalink() . '">' . 'View &raquo;' . '</a></div>'
        . '</div></div></div>';
    }
    wp_reset_query();
